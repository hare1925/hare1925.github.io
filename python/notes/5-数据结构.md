...
如果表达式是一个元祖（例如上面的（x， y）），那么就必须挂上括号
...
>>>>>>> d53f5b7926e751660031f61149ad22c309ebda10
...
有一种方式可以从列表按照给定的索引而不是值来移除一个袁术：那就是del语句。
它不同于会返回一个值的pop（）方法。
del语句也可以用来从列表中移除切片或者清空整个列表（我们之前用过的方式是将一个空列表赋值给指定的切片）。
例如：
