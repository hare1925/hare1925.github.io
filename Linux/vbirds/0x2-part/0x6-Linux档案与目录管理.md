...
由Linux档案权限的内容我们可以知道一个档案有若干个属性，包括读写执行（r、w、x），以及是否为目录（d）与档案（-）或者是连结档（l）等等的属性！  
要修改属性的方法（chgrp, chown, chmod）  

除了基本 r、 w、 x权限外，在Linux传统的 Ext2/Ext3/Ext4 档案系统下，我们还可以设定其他的系统隐藏属性，这部分可使用 **chattr 来设定，用 lsattr 来查看**,最重要的属性就是可以设定其不可修改的特性！  
连结档案的拥有者都不能尽心修改！这个属性是相当重要的，尤其是在安全机制上面（security）！  
比较可惜的是，在centos7 x 当中 利用给xfs 作为预设档案系统，但是 xfs 就没有支援所有的 chattr的参数了，仅有部分参数海鸥支援而已！  
...
在上面这个例题当中，如果知道755那个分数是怎么计算出来的，那么应该对于权限有一定程度的概念了。  
如果不知道 755 怎么来的，赶紧复习一下，如果对于权限认识的差不多了，那么就要聊聊【新增一个档案或目录时，预设的权限是什么？】这个议题了！  


### 4.1、文件默认权限：umask

OK!我们现在应该知道如何建立或者改变一个目录或档案的属性了，不过，建立一个心的档案或目录时，他的预设权限会是什么呢？那就与 umask 这个东西有关了。  
**umask就是指定【目前使用者在建立档案或目录时候的权限预设值】。**  
那么如何得知或设定 umask 呢？他的指定条件以底下的方式来指定：
...
查阅的方式有两种，一种可以直接输入 umask ，就可以看到数字形态的权限设定分数，一种则是加入 `-S` (Symbolic) 这个选项，就会以符号类型的方式来显示出权限了！  
奇怪的是，怎么 umask 会有四组数字啊？不是只有三组吗？  

**没错，狄自语是特殊权限用的**，我们先不要理他，所以先看后面三组即可。  
...
要注意的是， umask 的分数指的是【该预设值需要减掉的权限！】因为 r、w、x分别是 4、2、1分，所以，也就是说，当要拿掉能写的权限，就是输入2分，而如果拿掉能读的权限，也就是 4 分，那么要拿掉读写的权限，就是 6 分，而要拿掉执行与写入的权限，也就是 3 分。  
